...
from settings_constants import WHITE, map, damageFactor, WIDTH, HEIGHT, gap, thickness, defaultInterval, defaultPipeSpeed
  ...
  def update(self, pipes, pipeSpeed, playing = False):
    ...
    if not playing:
      ...
      if len(pipes_ahead) > 0:
        ...
        dist_next_pipe_normalized = map(self.y-pipes_ahead[0].y+self.height, 0,defaultInterval-thickness,-1,1)
